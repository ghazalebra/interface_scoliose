...
class MyApp(Widget):
    ...
    def detect_marqueurs(self):
        ...
        global images
        images = []
        for file in os.listdir(save_path_im):
            image = cv2.imread(os.path.join(save_path_im, file))
            images.append(image)

        global im_dim
        im_dim = images[0].shape

        for i, image in enumerate(images):
            key_points = marker_detection.detect_markers(image)
            key_points = [key_points[j] for j in range(len(key_points))]
            points = [[float(point.pt[0]), float(point.pt[1])] for point in key_points]

            dict_coordo[f'image{i+1}'] = points

        detection_eff = True

    def tapir(self):
        select_points = dict_coordo[f'image{image_nb}']
        timer_debut_detection = time.process_time_ns()
        height, width = im_dim[0], im_dim[1]

        frames = np.ndarray((images_total, height, width, 3))
        for i, image in enumerate(images):
            frames[i,:,:,:] = image
        ...
        print(f'Temps TAPIR :{timer_fin_detection - timer_debut_detection} ns')
...
class TAPIR():
    checkpoint_path = r'C:\Store\anaconda3\envs\PI3\interface_scoliose\tapnet\checkpoints\causal_tapir_checkpoint.npy'
    ...
    def inference(frames, query_points):
        ...
        frames = TAPIR.preprocess_frames(frames)
        ...
        outputs, _ = TAPIR.model_apply(TAPIR.params, TAPIR.state, rng, frames, query_points)
        outputs = tree.map_structure(lambda x: np.array(x[0]), outputs)
        tracks, occlusions, expected_dist = outputs['tracks'], outputs['occlusion'], outputs['expected_dist']

        # Binarize occlusions
        visibles = TAPIR.postprocess_occlusions(occlusions, expected_dist)
